...
module Def (F : Cstubs.FOREIGN) = struct
  ...
  let fd_zero =
    foreign "ocaml_posix_time_fd_zero"
      (ptr Posix_time_types.fd_set @-> returning void)

  let fd_set =
    foreign "ocaml_posix_time_fd_set"
      (int @-> ptr Posix_time_types.fd_set @-> returning void)

  let fd_isset =
    foreign "ocaml_posix_time_fd_isset"
      (int @-> ptr Posix_time_types.fd_set @-> returning int)

  let fd_clr =
    foreign "ocaml_posix_time_fd_clr"
      (int @-> ptr Posix_time_types.fd_set @-> returning void)

  let select =
    foreign "select"
      ( int
      @-> ptr Posix_time_types.fd_set
      @-> ptr Posix_time_types.fd_set
      @-> ptr Posix_time_types.fd_set
