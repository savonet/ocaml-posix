...
module Sys = struct
  type timeval = { tv_sec : int64; tv_usec : int64 }
  type itimerval = { it_interval : timeval; it_value : timeval }
  ...
  let from_itimerval { it_interval; it_value } =
    ...
    setf itimerval Types.Itimerval.it_interval (from_timeval it_interval);
    setf itimerval Types.Itimerval.it_value (from_timeval it_value);
    itimerval
        ...
        Errno_unix.raise_on_errno (fun () ->
            ...
            let timeval =
              match timeval with
                ...
                | Some timeval -> addr (from_timeval timeval)
            in
            match select (!maxfd + 1) r_set w_set e_set timeval with
              ...
              | _ ->
                  let get_fd_set l fd_set =
                    List.filter
                      (fun fd -> fd_isset (Obj.magic fd) fd_set <> 0)
                      l
                  in
                  Some
                    (get_fd_set r r_set, get_fd_set w w_set, get_fd_set e e_set)))
