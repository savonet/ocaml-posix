(* Generate get_system_value function for tests using errno_defs *)
let generate_get_system_value () =
  (* Generate cases from errno_defs (not errno_defaults) to get all errno names *)
  let regular_cases =
    List.map
      (fun (def : Errno_defaults.errno_def) ->
        Printf.sprintf "  | \"%s\" -> Posix_errno.to_int `%s" def.name def.name)
...
(* Generated from errno_defaults.mli using gen_get_system_value *)
